...
const toolTipHandleFunc = props => {
  ...
  return (
    <Tooltip
      prefixCls="component__rc_slider_tooltip rc-slider-tooltip"
      overlay={value}
      visible={dragging}
      placement="top"
      key={index}
...
const SliderBox = props => {
  ...
  return (
    <Slider
      ...
      max={max}
      step={stepper}
      defaultValue={slideDefaultValue}
...
const RangeBox = props => {
  ...
  return (
    <Range
      ...
      max={max}
      step={stepper}
      defaultValue={[defaultThresholdInit, defaultThresholdLast]}
      onChange={handleChangefunc}
      disabled={disabled}
...
export default ({ className, labelText, labelPosition, type, ...props }) => {
  ...
  const addAllClasses = ['component__rangebox'];
  ...
  const LabelField = labelText && (
    <span className="component__field-label">{labelText}</span>
  );
